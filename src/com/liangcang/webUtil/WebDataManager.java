...
public class WebDataManager {
    ...
    public static final String HOST = "api.taose69.com";
     ...
     * @throws IOException
     */
    public Map<String, String> generateApiParams() throws IOException {
        TreeMap<String, String> params = new TreeMap<String, String>( );
        params.put( "v", "2" );//  3 版本为googlePlay没有审核通过的版本
        params.put( "app_key", "android_mobile" );
        return params;
    }
     ...
     * @throws IOException
     */
    public String doPost(String url, Map<String, String> params, Map<String, String> headers, int connectTimeout, int readTimeout, boolean responseError) throws Exception {
        return doPost( url, params, headers, DEFAULT_CHARSET, connectTimeout, readTimeout, responseError );
    }

    public String doPost(String url, Map<String, String> params) throws Exception {
        return doPost( url, params, null, DEFAULT_CHARSET, connectTimeout, readTimeout, true );
    }

    public String doPost(String url, Map<String, String> params, int timeOut) throws Exception {
        return doPost( url, params, null, DEFAULT_CHARSET, timeOut, timeOut, true );
    }
     ...
     * @throws IOException
     */
    public String doPost(String url, Map<String, String> params, Map<String, String> headers, String charset, int connectTimeout, int readTimeout, boolean responseError)
            throws Exception {
     ...
     * @throws IOException
     */
    public String doPost(String url, String ctype, byte[] content, Map<String, String> headers, int connectTimeout, int readTimeout, boolean responseError) throws Exception {
        HttpURLConnection conn = null;
        OutputStream out = null;
        String rsp = null;
        try {
            try {
                conn = getConnection( /* context, */new URL( url.startsWith( "http" ) ? url : (ROOTPATH + url) ), METHOD_POST, ctype );
                if (headers != null) {
     ...
     * @throws IOException
     */
    public String doPost(String url, Map<String, String> params, Map<String, String> headers, Map<String, FileItem> fileParams, int connectTimeout, int readTimeout)
            throws Exception {
        if (fileParams == null || fileParams.isEmpty( )) {
            return doPost( /* context, */url, params, headers, DEFAULT_CHARSET, connectTimeout, readTimeout, false );
        } else {
            return doPost( /* context, */url, params, headers, fileParams, DEFAULT_CHARSET, connectTimeout, readTimeout );
        }
    }
     ...
     * @throws IOException
     */
    public String doPost(/* Context context, */String url, Map<String, String> params, Map<String, String> headers, Map<String, FileItem> fileParams, String charset,
    ...
    public String doGet(String url, Map<String, String> params, String charset) throws IOException {
        ...
        try {
            ...
            String query = buildQuery( params, charset );
            try {
                conn = getConnection( /* context, */buildGetUrl( url, query ), METHOD_GET, ctype );
            } catch (IOException e) {
                e.printStackTrace( );
            }
        ...
        MyLog.i( TAG, "do get url=" + url + " 耗时:" + (System.currentTimeMillis( ) - time) + " " + conn.getURL( ) );
        if (MyLog.isDEBUG)
            MyLog.i( TAG, "get date=" + rsp );
        return rsp;
    }

    public String downFile(String url, Map<String, String> params) throws IOException {
    ...
    private String getResposeMsg(HttpURLConnection conn, String errMsg) {
        if (conn != null) {
            try {
                ...
                // MyLog.e( TAG, "==========================" );
                // MyLog.e( TAG, "responseCode=" + responseCode + "   " + conn.getResponseMessage( ) );
                // MyLog.e( TAG, "==========================" );
                if (responseCode >= 500) {
    ...
    public String downFile(String fileName, String url, Map<String, String> params) throws IOException {
        HttpURLConnection conn = null;
        String rsp = null;
//        long time = System.currentTimeMillis( );
        try {
        ...
        // MyLog.i( TAG, "do get file=" + url + " 耗时:" + (System.currentTimeMillis( ) - time) );

        return rsp;
    }

    private HttpURLConnection getConnection(/* Context context, */URL url, String method, String ctype) throws IOException {
    ...
    public URL buildGetUrl(String url, Map<String, String> params, String charset) throws IOException {
    ...
    private URL buildGetUrl(String strUrl, String query) throws IOException {
        URL url = new URL( strUrl.startsWith( "http" ) ? strUrl : (ROOTPATH + strUrl) );
     ...
     * @throws IOException
     */
    protected String getResponseAsString(HttpURLConnection conn, boolean responseError) throws IOException {
        String charset = getResponseCharset( conn.getContentType( ) );
        String header = conn.getHeaderField( "Content-Encoding" );
        boolean isGzip = false;
        if (header != null && header.toLowerCase( Locale.getDefault( ) ).contains( "gzip" )) {
            isGzip = true;
        }
        ...
        if (es == null) {
            ...
            if (TextUtils.isEmpty( msg )) {
                throw new IOException( "ResponseCode=" + conn.getResponseCode( ) + ",ResponseMessage=" + conn.getResponseMessage( ) );
    ...
    protected String getResponseAsFile(HttpURLConnection conn, String fileName, boolean responseError) throws IOException {
    ...
    private String getStreamAsFileName(InputStream stream, String fileName) throws IOException {
        try {
