...
public class ImageDownloader {
    ...
    private Bitmap downloadBitmap(String url) {
        ...
        final HttpClient client = hasAndroidClinet ? AndroidHttpClient.newInstance( "Android" ) : new DefaultHttpClient( );
        long time = System.currentTimeMillis( );
        String addStr = "";
        addStr = (url.contains( "taobaocdn.com" ) && !isContainScale( url ) ? currentQuality.getValue( ) : "");
            ...
            if (entity != null) {
                ...
                try {
                    ...
                    if (CACHE) {
                        ...
                        // MyLog.d( "down", "耗时1=" + (System.currentTimeMillis( ) - time) );
                        if (path == null) {
                            bitmap = BitmapFactory.decodeStream( inputStream );
                        } else {
                            bitmap = BitmapFactory.decodeFile( path );
                        }
                        // MyLog.d( "down", "耗时2=" + (System.currentTimeMillis( ) - time) );
                    } else {
                        bitmap = BitmapFactory.decodeStream( inputStream );
                        // MyLog.d( "down", "耗时=" + (System.currentTimeMillis( ) - time) );
                    }
        ...
        MyLog.e( "down", "耗时：" + (System.currentTimeMillis( ) - time) + "url=" + url );
        return bitmap;
    }

    private boolean isContainScale(String url) {
        return url.endsWith( Quality.HIGHT.getValue( ) ) || url.endsWith( Quality.MIDDLE.getValue( ) ) || url.endsWith( Quality.LOW.getValue( ) )
                || url.endsWith( Quality.LOWEST.getValue( ) ) || url.endsWith( Quality.ORIGINAL.getValue( ) );
    }
    ...
    public String downloadPic(String url, Quality currentQuality) {
        ...
        final HttpClient client = hasAndroidClinet ? AndroidHttpClient.newInstance( "Android" ) : new DefaultHttpClient( );
        final HttpGet getRequest = new HttpGet( url + (currentQuality != null && url.contains( "taobao" ) ? currentQuality.getValue( ) : "") );
        String path = null;
        try {
            HttpResponse response = client.execute( getRequest );
            final int statusCode = response.getStatusLine( ).getStatusCode( );
